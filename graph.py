  ...
  def explore(self):
    ...
    while current_room != 'treasure room':
      ...
      for connected_room, weight in node.edges.items():
        ...
        print('Enter ' + str(key) + ' for ' + str(connected_room) + ': ' + str(weight) + ' cost')
      valid_choices = [room[:1] for room in node.edges.keys()]
      print('\nYou have accumulated: ' + str(path_total) + ' cost')
      choice = input("\nWhich room do you want to move to?\n")
      if choice not in valid_choices:
        print('Please select from these letters: ' + str(valid_choices))
      else:
        for room in node.edges.keys():
          if room.startswith(choice):
            current_room = room
            path_total += node.edges[room]
        print('\nYou have chosen: ' + str(current_room) + '\n')
    print('Made it to the treasure room with ' + str(path_total) + ' cost.')   
